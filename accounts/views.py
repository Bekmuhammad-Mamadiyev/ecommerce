...
class VerifyOtpView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            sms_obj.save()
            return Response(data={'message':"User verified successfully", "verification":sms_obj.id})
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            VerifictionOtp.objects.create(user=user, type=VerifictionOtp.VerificationType.RESET_PASSWORD,
                                           code=code,
                                           expires_in=timezone.now() + timezone.timedelta(minutes=settings.OTP_CODE_ACTIVATION_TIME))
            send_email(code=code, email=user.email)
            return Response(data={'message':"Reset password link sent successfully",}, status=status.HTTP_200_OK)
        except User.DoesNotExist:
...
class ResetPasswordFinishView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            user = User.objects.get(email=data.get('email'))
            sms_code = VerifictionOtp.objects.get(user = user, type=VerifictionOtp.VerificationType.RESET_PASSWORD, id=data.get('verification'))
            if sms_code.is_active is True:
                return Response(data={"message":"otp code is not activated_yet"}, status=status.HTTP_400_BAD_REQUEST)
            user.set_password(data.get('password'))
            user.save()

            return Response(data={"message":"password set successfully"}, status=status.HTTP_200_OK)
        except User.DoesNotExist:
            raise APIException('User does not exist')
        except VerifictionOtp.DoesNotExist:
            raise APIException('Verification code does not exist')